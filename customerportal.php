...
$server->register(
	...
	array('return'=>'tns:common_array'),
	$NAMESPACE);
//Tax Details of Admin Settings :: Manasa added 30th oct 2018
$server->register('get_product_shipping_taxes',
     array(),
	array('return'=>'tns:common_array'),
	$NAMESPACE);
$server->register(
...
function documents_ticket_list($input_array)
{	

	global $adb,$log; 
	$query = "SELECT * FROM `vtiger_notes` INNER JOIN vtiger_senotesrel ON vtiger_senotesrel.notesid = vtiger_notes.notesid WHERE vtiger_senotesrel.crmid =".$input_array['ticket_id'];
	 $result = $adb->pquery($query, array());

	 $finaldata = array();
        while($row = $adb->fetchByAssoc($result)){ 
        	$row['file_path'] = getImagePath($row['notesid']);
            $finaldata[] = $row;
        }  
	return  $finaldata;
}

function update_list_ticket($input_array)
{	
...
function change_ticket_status($input_array)
{	global $adb;
	 $sql ="UPDATE vtiger_troubletickets 
	 		SET status = 'Closed' WHERE ticketid =".$input_array['ticketid'];
  	$result = $adb->pquery($sql, array());
	return  array("status"=>"success");
}

function check_session_id($input_array)
{	global $adb;
	 // $sql ="UPDATE vtiger_troubletickets 
	 // 		SET status = 'Closed' WHERE ticketid =".$input_array['sessionid'];
  // 	$result = $adb->pquery($sql, array());
	// return  array("status"=>$input_array['sessionid']);
	$id = $input_array['id'];
	...
	if(!validateSession($id,$sessionid))
	 ...
	 return  array("status"=>validateSession($id,$sessionid), "success"=>"user valid");
}


function create_ticket_comment($input_array)
{
...
function _getTicketModComments($ticketId) {
	...
	for($i=0; $i<$rows; $i++) {
		...
		if(!empty($customer)) {
			$emailResult = $adb->pquery('SELECT vtiger_portalinfo.*, vtiger_crmentity.label FROM vtiger_portalinfo 
				LEFT JOIN vtiger_crmentity on vtiger_crmentity.crmid = vtiger_portalinfo.id
				WHERE vtiger_portalinfo.id = ?', array($customer));
			$output[$i]['owner'] = $adb->query_result($emailResult, 0 ,'label');
		} else {
		...
		$file_name = $adb->query_result($result, $i, 'filename');
		$modcommentsid = $adb->query_result($result, $i, 'modcommentsid');
...
function get_tickets_list($input_array) {
	...
	while($row = $adb->fetchByAssoc($res)) {
	  ...
	  $output['travel'] = $folders;
	$noofdata = $adb->num_rows($res);

	if($noofdata == 0){
		$output['travel'][0]['rows']=0;
	}elseif ($noofdata == 1) {
		$output['travel'][0]['rows']=1;
	}elseif ($noofdata > 1) {
		$output['travel'][0]['rows']=2;
	}
	...
	{	
		// $output[0]['final'][$j] =$fields_list[$j];
		$i=0;
		foreach($fields_list as $fieldlabel => $fieldname) {
...
function add_travelquote_record($input_array){
	...
	$totalproducts = $input_array['productsinfo']['productid'];
	foreach($taxes as $k=>$tdet){
		$taxinfo[$tdet['taxname']]=$tdet['percentage'];
	}
	foreach($totalproducts as $keypro=>$valuepro){
		$pid = $valuepro;
		$pqty = $input_array['productsinfo']['product_qty'][$keypro];		
		...
		$adb->println($productinfo['unit_price']);
		$lineItem[] = array_merge(array('productid'=>$proid,'listprice'=>$productinfo['unit_price'],'quantity'=>$pqty),$taxinfo);
	}
	$subject = $parentRecordModel->getName().' From Portal';
	$current_user = $user->retrieveCurrentUserInfoFromFile(Users::getActiveAdminId());
	try {
		$defaultAssignee = getDefaultAssigneeId();
		$userid = vtws_getWebserviceEntityId('Users', $defaultAssignee); 
		$currencyid = vtws_getWebserviceEntityId('Currency', '2');
		$data = array (
			   'subject' => $subject,
			   'quotestage'=> 'Created',
...
function send_mail_for_password($mailid)
{
	...
	if ($user_name == '' && $password == '') {
		$ret_msg = "Invaild email id";
	} elseif ($isactive == 0) {
		$ret_msg = "Login expired";
	} else {
...
function add_ticket_attachment($input_array)
{
	...
	$filetmp = $input_array['filetmp'];
	$title = $input_array['title'];
	if($title == ''){
		$title = $filename;
	}
	$upload_filepath = decideFilePath();
	$attachmentid = saveAttachment($filename,$filetmp,$filetype);
 ...
 function saveAttachment($fName,$fileTmpName,$fileType) {
		...
		if(!empty($fileName)) {
			...
			$filepath = $uploadPath.$attachid . "_" . $binFile;
			$filecontents = base64_encode(file_get_contents('SoapTest/tmp/'.$fName));
			$data = base64_decode($filecontents);
			$handle = @fopen($filepath,'w');
			fputs($handle, $data);
			fclose($handle);
			if(file_exists('SoapTest/tmp/'.$fName)){
				unlink('SoapTest/tmp/'.$fName);
			}
 ...
 *
 */
function get_product_list_values($id,$modulename,$only_mine='true')
{
	...
	$query = array();
	$params = array();
	$query = "SELECT vtiger_products.*,vtiger_crmentity.*,vtiger_seproductsrel.crmid as entityid, vtiger_seproductsrel.setype FROM vtiger_products
...
/*	$checkQuotes = checkModuleActive('Quotes');
	...
	//{
		...
		for( $j= 0;$j < $noofdata; $j++)
		{
			...
			$prodcutsinfo['description'] =  $adb->query_result($res,$j,'description');
...
//Invoices
function get_invoices($id,$only_mine='true')
{
	$modulename = "Invoice";
	require_once('modules/Products/Products.php');
	require_once('include/utils/UserInfoUtil.php');
...
//Ended here
function get_quotes($id,$only_mine='true')
{
	$modulename = "Quotes";
	require_once('modules/Products/Products.php');
	require_once('include/utils/UserInfoUtil.php');
...
//Tax Details Retrive
function get_product_shipping_taxes(){
	global $log;
	$log->debug("Entered get_product_shipping_taxes...");
	require_once 'include/utils/InventoryUtils.php';
	$taxes =getAllTaxes('available');
	$shippingTaxes =getAllTaxes('available','sh');
	$output[]['producttaxesget_product_shipping_taxes'] = $taxes;
	$output[]['shippingtaxes'] = $shippingTaxes;
	$log->debug("exit ...");
	return $output;
}
//Ended here
